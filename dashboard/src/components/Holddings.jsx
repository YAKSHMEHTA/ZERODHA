  ...
  return (
    <div className="holdings-container">
      ...
      <div className="summary">
        <div>Investment: ₹{parseFloat(summary.totalInvested).toLocaleString('en-IN')}</div>
        <div>Current: ₹{parseFloat(summary.totalCurrent).toLocaleString('en-IN')}</div>
        <div className={parseFloat(summary.totalPnL) >= 0 ? 'profit' : 'loss'}>
          P&L: ₹{parseFloat(summary.totalPnL).toLocaleString('en-IN')} ({summary.totalPnLPercent}%)
        </div>
      </div>
